...
const ShopContent: React.FC<ShopContentProps> = ({ products }) => {
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(product => {
      ...
      // Filtre par hauteur
      if (filters.height.length > 0 && !filters.height.includes(product.height)) {
        return false;
      }
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {filters.height.map(height => (
                <span key={height} className="bg-purple-100 text-purple-800 px-2 py-1 rounded text-xs flex items-center">
                  {height}
                  <button
                    onClick={() => setFilters({
                      ...filters,
              ...
              {filters.color.map(color => (
                <span key={color} className="bg-yellow-100 text-yellow-800 px-2 py-1 rounded text-xs flex items-center">
                  {color}
                  <button
                    onClick={() => setFilters({
                      ...filters,
