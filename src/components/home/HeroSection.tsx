...
async function getHeroContent() {
  try {
    ...
    const response = await fetch(`${baseUrl}/api/hero-content`, {
      cache: "no-store" // Pour toujours récupérer les données fraîches
      // OU
      // next: { revalidate: 60 } // Pour mettre en cache pendant 60 secondes
    })
...
export default async function HeroSection() {
  ...
  // Données par défaut (fallback) au cas où la base de données ne répond pas
